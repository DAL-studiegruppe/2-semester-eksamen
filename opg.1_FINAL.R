...
# glm graf
glm_matrix <- table(preds_glm, test$SHOTISGOAL)
...
rf <- randomForest(SHOTISGOAL ~ distance_to_goal + angle_to_goal + is_foot + LOCATIONX + LOCATIONY, 
...
plot(rf,log="x",main="black default, red samplesize, green tree depth")

roc_curve_rf <- roc(training$SHOTISGOAL, rf_probs[,2])
roc_curve_rf <- plot(roc_curve_rf, print.auc=TRUE, auc.polygon=TRUE, grid=TRUE)
# kontrol
...
# Find optimal threshold (maksimerer sum af sensitivitet og specificitet)
optimal_threshold <- c(coords(roc_curve, "best", ret = "threshold"))

# Anvend optimal threshold
optimal_preds <- ifelse(rf_probs_test[,2] > optimal_threshold, 1, 0)
table(optimal_preds,test$SHOTISGOAL)
...
server <- function(input, output) {
  ...
  output$plot1 <- renderPlot({
    roc_curve_rf <- roc(training$SHOTISGOAL, rf_probs[,2])
    roc_curve_rf <- plot(roc_curve_rf, 
